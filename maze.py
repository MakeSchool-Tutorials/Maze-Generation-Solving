...
class Maze:
    ...
    def solution_array(self):
        ...
        while current_index != self.total_cells - 1:
            next_cell_bits = (self.maze_array[current_index] &
                              SOLUTION_BITS) >> 8
            try:
                i = WALLS.index(next_cell_bits)
            except ValueError:
                return 'ERROR - SOLUTION BITS INVALID!'
            solution.append(DIRECTION[i])
            x, y = self.x_y(current_index)
            new_x = x + COMPASS[i][0]
            new_y = y + COMPASS[i][1]
            current_index = self.cell_index(new_x, new_y)
